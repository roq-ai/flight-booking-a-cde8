...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiAirplay } from 'react-icons/fi';

import { useAirlineFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['sales-manager', 'business-owner', 'team-member', 'finance-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useAirlineFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Airlines',
      path: isTenantUser ? `/airlines/view/${data?.id}` : '/airlines',
      entity: 'airline',
